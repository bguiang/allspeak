...
const connect = (event) => {
  ...
  language = document.querySelector("#language").value.trim();
  ...
  if (username && language) {
    ...
    const socket = new SockJS("/allspeak");
...
const onConnected = () => {
  console.log("ASDLKHJASKLJDHSA:  " + "/topic/" + language);
  stompClient.subscribe("/topic/public", null);
  stompClient.subscribe("/topic/" + language, onMessageReceived);
  stompClient.send(
    "/app/chat.newUser",
    {},
